...
const EditDiscount = ({ isOpen, onClose, item, onUpdateDiscount, existingPromo }) => {
  ...
  // Load existing promotion data when component mounts or when existingPromo changes
  useEffect(() => {
    if (existingPromo && isOpen) {
      setDiscountData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onUpdateDiscount(item, discountData);
    } catch (error) {
...
EditDiscount.propTypes = {
  ...
  existingPromo: PropTypes.object.isRequired,
};
